...
#include "extlibs/StatisticsTools/multitau-correlator.h"
...
class FCSMeasurement: public FluorescenceMeasurement, public RelativeAbsorbanceReader {
    ...
    protected:
         ...
         *
         * This function calculates \f[ \max\limits_{z}\left[\iint\limits_{-\infty}^\infty f(\sqrt{x^2+y^2}, z)\;\mathrm{d}x\;\mathrm{d}y\right]=\max\limits_{z}\left[\sum\limits_{r=0}^{r_{max}} f(r, z)\cdot \left(\pi(r+1)^2-\pi r^2\right)\cdot\mbox{psf\_rz\_image\_rresolution}^2 \right] \f]
         ...
         */
         ...
         /*! \brief detection propability distribution
            ...
            possible values:
              ...
              - 3: gaussian beam pixel: the same as 2 ("gaussian beam") but with a square pixel (uses additional parameter pixel_size) \f[ \mbox{PSF}(x,y,z)=\frac{\iint\limits_{-\mbox{pixel\_size}/2}^{+\mbox{pixel\_size}/2}f(\sqrt{x^2+y^2}, z)\;\mathrm{d}x\;\mathrm{d}y}{\max\limits_z\iint\limits_{-\infty}^\infty f(\sqrt{x^2+y^2}, z)\;\mathrm{d}x\;\mathrm{d}y} \f] with \f[ f(r,z)=\left(\frac{\mbox{detpsf\_r0}}{W(z, \mbox{detpsf\_z0}, \mbox{detpsf\_r0})}\right)^2\cdot\exp\left(-2\cdot\frac{r^2}{W(z, \mbox{detpsf\_z0}, \mbox{detpsf\_r0})^2}\right) \f] Here the 2D-integral over the pixel is calculated as described in the main documentation of this class
         */
